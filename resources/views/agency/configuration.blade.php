    ...
    <div class="card mb-9">
        <div class="card-header pt-7 ">
            ...
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bold text-gray-800 ">Set Custom Domain {{ tenant()->domains()->first()->changed == 1 ? '(Changed)' : '' }}</span>
                <span class="text-gray-500 mt-1 fw-semibold fs-6">This is the URL that this platform uses, you can only change it once.</span>
            </h3>
        </div>
        <!--begin::Body-->
...
<!--begin::Form-->
<form action="{{ route('merchant.configuration.domainupdate') }}" class="form mb-9" method="post">
    @csrf
...
<div class="row mb-9">
    <div class="col-md-12 fv-row">
        <!--begin::Label-->
        <label class="required fs-5 fw-semibold mb-2">Website Domain</label>
        <!--end::Label-->
        <!--begin::Input-->
        <input type="text" class="form-control form-control-solid" name="custom_domain" value="{{ tenant()->domains()->first('domain')->domain ?? 'No domain set' }}" {{ tenant()->domains()->first()->changed == 1 ? 'readonly' : '' }}/>
        <!--end::Input-->
...
<div class="row mb-5">
    <div class="col-md-6 fv-row">
        <!--begin::Label-->
        <label class="required fs-5 fw-semibold mb-2">Minimum payout amount({{ $setCurrency }})</label>
        <!--end::Label-->
...
<div class="row mb-5">
    <div class="col-md-6 fv-row">
        <!--begin::Label-->
        <label class="required fs-5 fw-semibold mb-2">Payout method</label>
        <!--end::Label-->

        <!--begin::Input-->
        <select class="form-control form-control-solid" name="payout_methods"  aria-label="" data-control="">
            <option value="">Select all you support</option>
            @foreach ($payoutMethods as $payoutMethod)
                <option value="{{ $payoutMethod->id }}" {{ ($payoutMethod->id  ==  settings()->get('payout_methods')) ? 'selected' : '' }}>{{ $payoutMethod->processor }}</option>
            @endforeach
        </select>
        <!--end::Input-->
    </div>
